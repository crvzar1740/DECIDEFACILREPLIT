  ...
  return (
    <div className="flex rounded overflow-hidden h-8 bg-black/30 border border-white/[0.06]">
      {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
        <button key={n} onClick={() => onChange(n)} className={`flex-1 text-[10px] font-semibold border-r border-white/[0.04] last:border-r-0 transition-colors ${value === n ? accent : "text-slate-500 hover:bg-white/[0.04] hover:text-slate-300"}`}>{n}</button>
      ))}
    </div>
  );
}
...
function Asistente({ onBack }: { onBack: () => void }) {
  ...
  const [veg, setVeg] = useState(""); const [prot, setProt] = useState(""); const [carb, setCarb] = useState("");
  const [audProt, setAudProt] = useState(false); const [audVeg, setAudVeg] = useState(false); const [audCarb, setAudCarb] = useState(false);

  // Tiempo state
  const [matutino, setMatutino] = useState(""); const [gestion, setGestion] = useState(""); const [deep, setDeep] = useState("");
  const [dosmin, setDosmin] = useState(""); const [pareto, setPareto] = useState("");

  // Ocio state
  // FAST state
  const [fatidica, setFatidica] = useState(""); const [alinea, setAlinea] = useState(""); const [solucion, setSolucion] = useState(""); const [tranquilidad, setTranquilidad] = useState("");
    ...
    if (c === "comida") setEvalContent(
      <div className="space-y-3">
        <div className="p-3 bg-white/[0.02] border border-white/[0.05] rounded-xl">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">Implementá un <strong>Menú Rotativo</strong> o la <strong>Fórmula del Plato Inteligente</strong> para eliminar la fatiga por decisión nutricional.</p>
        </div>
    ...
    setEvalContent(
      <div className="space-y-3">
        <div className="p-3 bg-white/[0.02] border border-white/[0.05] rounded-xl space-y-2">
          {[["🥗 Vegetales (50%):", v, "text-emerald-400"], ["🍗 Proteínas (25%):", p, "text-orange-400"], ["🥔 Carbohidratos (25%):", c, "text-sky-400"]].map(([label, val, col]) => (
            <div key={label as string} className="flex justify-between text-xs"><span className={col as string}>{label as string}</span><span className="text-white">{val ? "Asignado ✓" : "Faltante ✗"}</span></div>
          ))}
        </div>
  ...
  const updateBloques = (m: string, g: string, d: string) => {
    setEvalContent(
      <div className="space-y-2">
        ...
        {[["🌅 Matutino", m || "—", "text-orange-400"], ["📊 Gestión", g || "—", "text-sky-400"], ["🚀 Deep Work", d || "—", "text-purple-400"]].map(([label, val, col]) => (
          <div key={label as string} className="p-2.5 bg-black/30 border border-white/[0.05] rounded-xl"><span className={`text-[10px] font-bold ${col}`}>{label as string}</span><p className="text-xs text-white mt-0.5">{val as string}</p></div>
        ))}
      </div>
    );
  };
        ...
        <div className="lg:col-span-2 bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 space-y-6">
            ...
            <div className="space-y-5 pt-4 border-t border-white/[0.05]">
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  {[["50% VEGETALES", "text-emerald-400", veg, setVeg, [["Hojas verdes","Hojas verdes (Espinaca, rúcula)"],["Fibrosos","Vegetales fibrosos (Brócoli, coliflor)"]]], ["25% PROTEÍNAS", "text-orange-400", prot, setProt, [["Pollo / Pavo","Pollo / Pavo magro"],["Pescado","Pescado (Salmón/Atún)"],["Huevos","Huevos"],["Legumbres","Legumbres / Tofu"]]], ["25% CARBOHIDRATOS", "text-sky-400", carb, setCarb, [["Arroz / Quinoa","Arroz integral / Quinoa"],["Papa / Batata","Papa / Batata al horno"],["Avena","Avena / Legumbres"]]]]
                    .map(([label, col, val, setter, opts]: any) => (
                      <div key={label} className="p-3 bg-black/40 border border-white/[0.05] rounded-xl space-y-2">
                        <span className={`text-[10px] font-bold block ${col}`}>{label}</span>
                        <select value={val} onChange={e => { setter(e.target.value); auditarPlato(label.includes("VEG") ? e.target.value : veg, label.includes("PROT") ? e.target.value : prot, label.includes("CARB") ? e.target.value : carb); }} className="w-full bg-[#0A0F1D] text-xs text-slate-200 border border-white/10 rounded p-1.5 outline-none">
                          <option value="">-- Selecciona --</option>
                          {opts.map(([k, v]: string[]) => <option key={k} value={v}>{k}</option>)}
                        </select>
                      </div>
                    ))}
                </div>
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-3 gap-2">
                  {[["Tengo una Proteína", audProt, setAudProt], ["Tengo Vegetales", audVeg, setAudVeg], ["Tengo Carbohidratos", audCarb, setAudCarb]].map(([label, checked, setter]: any) => (
                    <label key={label} className="flex items-center gap-2.5 p-2.5 bg-black/30 border border-white/[0.05] rounded-xl cursor-pointer">
                      <input type="checkbox" checked={checked} onChange={e => { setter(e.target.checked); const np = label.includes("Prot") ? e.target.checked : audProt; const nv = label.includes("Veg") ? e.target.checked : audVeg; const nc = label.includes("Carb") ? e.target.checked : audCarb; if (np && nv && nc) setEvalContent(<div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl space-y-2"><div className="flex items-center gap-2 text-emerald-400 font-bold text-xs"><CheckCircle className="w-4 h-4" /> Criterio Mínimo Cubierto</div><p className="text-xs text-slate-300 leading-relaxed">Combinalos con la <strong>Técnica del Bowl</strong> o el <strong>Plato Único</strong>. Añadí aceite de oliva y almorzá sin fatiga mental.</p></div>); }} className="w-4 h-4 accent-amber-400" />
          ...
          {cat === "tiempo" && (
            <div className="space-y-5 pt-4 border-t border-white/[0.05]">
              <div>
                ...
                <div className="grid sm:grid-cols-3 gap-2">
                  {[["Matriz Eisenhower","📊 MATRIZ EISENHOWER","1. Urgente+Importante (Haz ya). 2. Importante No Urgente (Agenda). 3. Urgente No Importante (Delega). 4. Elimina distractores.","text-sky-400","bg-sky-500/5 border-sky-500/10"],["Método Ivy Lee","📝 MÉTODO IVY LEE","Escribe estrictamente 6 tareas cruciales para mañana. Trabaja en la #1 de forma exclusiva.","text-purple-400","bg-purple-500/5 border-purple-500/10"],["Comerse el Sapo","🐸 COMERSE EL SAPO","Identifica la tarea más pesada del día y ejecútala a primera hora de la mañana.","text-orange-400","bg-orange-500/5 border-orange-500/10"]].map(([label, title, desc, col, bg]) => (
                    <button key={label} onClick={() => setEvalContent(<div className={`p-4 rounded-xl border ${bg}`}><p className={`text-[10px] font-bold uppercase ${col}`}>{title}</p><p className="text-xs text-slate-300 mt-1 leading-relaxed">{desc}</p></div>)} className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-xs font-semibold text-white transition-all">{label}</button>
                  ))}
                </div>
              </div>
              <div className="bg-black/20 border border-white/[0.05] rounded-xl p-4 space-y-3">
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  {[["bloque-matutino","🌅 Matutino / Acción Profunda...",matutino,setMatutino],["bloque-gestion","📊 Gestión / Reactivo...",gestion,setGestion],["bloque-deep","🚀 Deep Work de Cierre...",deep,setDeep]].map(([id,ph,val,setter]: any) => (
                    <input key={id} type="text" value={val} onChange={e => { setter(e.target.value); updateBloques(id === "bloque-matutino" ? e.target.value : matutino, id === "bloque-gestion" ? e.target.value : gestion, id === "bloque-deep" ? e.target.value : deep); }} placeholder={ph} className="w-full bg-[#0A0F1D] border border-white/10 text-xs rounded px-3 py-2 text-slate-200 outline-none focus:border-amber-400/40 transition-colors" />
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="bg-black/30 border border-white/[0.05] rounded-xl p-4 space-y-3">
                  {[["¿Toma menos de 2 minutos?",dosmin,setDosmin,[["si","Sí, de inmediato (< 2 min)"],["no","No, requiere más tiempo"]]],["¿Pertenece al 20% estratégico? (Pareto)",pareto,setPareto,[["si","Sí, es estratégico (80/20)"],["no","No, es secundario operativo"]]]].map(([label,val,setter,opts]: any) => (
                    <div key={label} className="grid grid-cols-3 items-center gap-2">
                      <label className="col-span-2 text-xs text-slate-300"><strong>{label}</strong></label>
                      <select value={val} onChange={e => { setter(e.target.value); const nd = label.includes("2 min") ? e.target.value : dosmin; const np = label.includes("Pareto") ? e.target.value : pareto; if (nd === "si") setEvalContent(<div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl"><p className="text-[10px] font-bold text-emerald-400 uppercase">⚡ Acción Inmediata</p><p className="text-xs text-slate-300 mt-1">Toma menos de 2 minutos. Ejecútalo ya sin agendar.</p></div>); else if (np === "si") setEvalContent(<div className="p-4 bg-sky-500/5 border border-sky-500/10 rounded-xl"><p className="text-[10px] font-bold text-sky-400 uppercase">🎯 Actividad 80/20</p><p className="text-xs text-slate-300 mt-1">Pertenece al 20% estratégico. Muévelo al próximo bloque Deep Work.</p></div>); else if (nd === "no" && np === "no") setEvalContent(<div className="p-4 bg-purple-500/5 border border-purple-500/10 rounded-xl"><p className="text-[10px] font-bold text-purple-400 uppercase">⏳ Rechazado al Bloque Reactivo</p><p className="text-xs text-slate-300 mt-1">No es estratégico ni rápido. Agenda para el bloque reactivo de la tarde.</p></div>); }} className="bg-[#0A0F1D] text-xs border border-white/10 text-slate-200 rounded p-1.5 outline-none">
            ...
            <div className="space-y-5 pt-4 border-t border-white/[0.05]">
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-2.5">
                  ...
                  <button onClick={() => setEvalContent(<div className="p-4 bg-sky-500/5 border border-sky-500/10 rounded-xl"><p className="text-[10px] font-bold text-sky-400 uppercase">✌️ Segunda Opción</p><p className="text-xs text-slate-300 mt-1">Cuando dudes entre opciones, descartá instantáneamente tu primera alternativa y ve a la segunda.</p></div>)} className="p-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-xs font-semibold text-white flex items-center justify-between transition-all"><span>✌️ Regla de la "Segunda Opción"</span><span className="bg-sky-500/20 text-sky-400 text-[10px] px-2 py-0.5 rounded">Algoritmo</span></button>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[["https://www.netflix.com/tudum/top10","🔴 Netflix Top 10","hover:border-red-500/40"],["https://www.imdb.com/chart/top/","🟡 IMDb Top 250","hover:border-amber-500/40"],["https://letterboxd.com/films/by/rating/","🟢 Letterboxd","hover:border-emerald-500/40"]].map(([url,label,hov]) => (
                    <a key={label as string} href={url as string} target="_blank" rel="noopener noreferrer" className={`p-2 bg-black/40 border border-white/[0.05] ${hov} rounded-xl text-xs flex items-center justify-center text-slate-300 transition-colors`}>{label as string}</a>
                  ))}
                </div>
              </div>
              <div>
          ...
          {cat === "fast" && (
            <div className="space-y-4 pt-4 border-t border-white/[0.05]">
              ...
              <div className="bg-black/30 border border-white/[0.05] rounded-xl p-4 space-y-3">
                {[["1. ¿Es Fatídica?","Irreversible/Alto riesgo",fatidica,setFatidica,[["no","No, es reversible"],["si","Sí, es irreversible"]]],["2. ¿Se Alinea a largo plazo?","",alinea,setAlinea,[["si","Sí, apoya mis metas"],["no","No, es distracción"]]],["3. ¿Existe solución predeterminada?","",solucion,setSolucion,[["no","No, requiere decisión"],["si","Sí, aplicar regla/delegar"]]],["4. ¿Trae Tranquilidad o Tensión?","",tranquilidad,setTranquilidad,[["tranquilidad","Tranquilidad / Alivio"],["tension","Tensión / Incomodidad"]]]].map(([label,hint,val,setter,opts]: any) => (
                  <div key={label} className="grid grid-cols-3 items-center gap-2 pt-2 first:pt-0 border-t border-white/[0.03] first:border-0">
                    <label className="col-span-2 text-xs text-slate-300"><strong>{label}</strong>{hint && <span className="text-slate-500 font-normal"> ({hint})</span>}</label>
                    <select value={val} onChange={e => { setter(e.target.value); calcFast(label.includes("Fatídica") ? e.target.value : fatidica, label.includes("Alinea") ? e.target.value : alinea, label.includes("solución") ? e.target.value : solucion, label.includes("Tranquilidad") ? e.target.value : tranquilidad); }} className="bg-[#0A0F1D] text-xs border border-white/10 text-slate-200 rounded p-1.5 outline-none">
                      <option value="">-- Seleccionar --</option>
                      {opts.map(([v,l]: string[]) => <option key={v} value={v}>{l}</option>)}
                    </select>
                  </div>
...
function Frameworks({ onBack }: { onBack: () => void }) {
  ...
  const [ebdmDatos, setEbdmDatos] = useState(""); const [ebdmExp, setEbdmExp] = useState(""); const [ebdmVal, setEbdmVal] = useState("");
  ...
  return (
    <div>
          ...
          <EvalBox>
            ...
            <div className="pt-3 border-t border-white/[0.05]">
              {[["≥150","Excelente. Alta alineación.","text-emerald-400"],["100–149","Buena opción. Revisá criterios.","text-sky-400"],["50–99","Mediocre. Reconsiderá.","text-amber-400"],["<50","Descarta. No alineada.","text-red-400"]].map(([r,l,c])=>(
                <div key={r as string} className="flex justify-between py-1.5 border-b border-white/[0.04] last:border-0"><span className={`text-xs font-bold ${c}`}>{r as string}</span><span className="text-xs text-slate-400">{l as string}</span></div>
              ))}
            </div>
          </EvalBox>
        </div>
      ...
      {sub === "ebdm" && (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 space-y-6">
            {[["1. Definir el problema", ebdmP1, setEbdmP1, "Describe el problema en una sola oración objetiva. Separa hechos de emociones..."]].map(([label,val,setter,ph]: any)=>(
              <div key={label}><label className="text-xs font-bold text-white uppercase block mb-2">{label}</label><input type="text" value={val} onChange={e=>setter(e.target.value)} placeholder={ph} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-xs text-white outline-none focus:border-sky-400/40 transition-colors" /></div>
            ))}
            <div>
              <label className="text-xs font-bold text-white uppercase block mb-2">2. Recopilar evidencia disponible (Los 3 Pilares)</label>
              <div className="grid sm:grid-cols-3 gap-3">
                {[["Datos objetivos / Métricas...", ebdmDatos, setEbdmDatos],["Experiencia profesional previa...", ebdmExp, setEbdmExp],["Valores, prioridades y límites reales...", ebdmVal, setEbdmVal]].map(([ph,val,setter]: any) => (
                  <textarea key={ph} value={val} onChange={e=>setter(e.target.value)} placeholder={ph} rows={4} className="w-full bg-[#0A0F1D] border border-white/10 rounded-lg p-2 text-xs text-white resize-none outline-none focus:border-sky-400/40 transition-colors" />
                ))}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {[["¿En 10 minutos?","¿En 10 meses?","¿En 10 años?"]].flat().map((ph, i) => (
...
// ─────────────────────────────────────────────────────────────────────────────
type AutoRow = { label: string; key: string; ph: string; type?: "select" };
...
function Automatizacion({ onBack }: { onBack: () => void }) {
  ...
  const FIELD_KEYS = ["desayuno","ropa","tarea","almuerzo","filtro","reunion","cena","pantalla","preparar"];
                ...
                {section.rows.map(row => (
                  <div key={row.key} className="grid grid-cols-3 items-center gap-2">
                    ...
                    {row.type === "select" ? (
                      ...
                      <input type="text" value={fields[row.key] || ""} onChange={setF(row.key)} placeholder={row.ph} className="col-span-2 bg-black/40 border border-white/10 rounded-xl p-2.5 text-xs text-white outline-none focus:border-purple-400/40 transition-colors" />
                    )}
                  </div>
                ))}
              </div>
        ...
        <EvalBox>
          ...
          <div className="border-t border-white/[0.05] pt-3">
            {[["8–10","Alta autoconciencia.","Criterio claro.","text-emerald-400"],["5–7","Nivel funcional.","Trabajá tus patrones.","text-sky-400"],["1–4","Baja claridad.","Necesitás estructura.","text-red-400"]].map(([r,l,s,c])=>(
              <div key={r as string} className="flex justify-between items-start py-2 border-b border-white/[0.04] last:border-0"><span className={`text-xs font-bold ${c} w-10`}>{r as string}</span><span className="text-xs text-slate-400 text-right"><strong className="text-slate-300 block">{l as string}</strong>{s as string}</span></div>
            ))}
          </div>
...
function Home({ onSelect }: { onSelect: (t: Tab) => void }) {
  return (
    <div className="space-y-10">
      ...
      <div className="grid sm:grid-cols-2 gap-6 max-w-5xl mx-auto pb-10">
...
export default function Herramientas() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden selection:bg-sky-400/20">
      ...
      {/* NAV */}
      <nav className="sticky top-0 z-20 bg-background/90 backdrop-blur-md border-b border-white/[0.05]">
